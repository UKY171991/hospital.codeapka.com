<?php
// doctors_example.php - demo page showing modal CRUD via AJAX
...
<div class="container">
    <div class="row mb-3">
        <div class="col-md-8"><h2>Doctors (Example)</h2></div>
        <div class="col-md-4 text-right"><button id="addBtn" class="btn btn-primary">Add Doctor</button></div>
    </div>
    <div class="card">
        <div class="card-body">
            <table class="table table-bordered" id="doctorTable">
                <thead><tr><th>ID</th><th>Name</th><th>Specialization</th><th>Email</th><th>Added By</th><th>Actions</th></tr></thead>
                <tbody></tbody>
            </table>
        </div>
    </div>
...
$(function(){
  ...
  $('#doctorTable').on('click','.edit-btn', function(){ var id=$(this).data('id'); $.get('ajax/doctor_api.php',{action:'get',id:id}, function(r){ if(r.success){ var d=r.data; $('#doctorId').val(d.id); $('#name').val(d.name); $('#qualification').val(d.qualification); $('#specialization').val(d.specialization); $('#phone').val(d.phone); $('#email').val(d.email); $('#address').val(d.address); $('#registration_no').val(d.registration_no); $('#doctorModal').modal('show'); } },'json'); });
