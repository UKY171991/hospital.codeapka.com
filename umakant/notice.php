...
$(function(){
  ...
  // Save handler
  $('#saveNoticeBtn').click(function(){ var data=$('#noticeForm').serialize()+'&action=save'; $.post('ajax/notice_api.php', data, function(resp){ console.log('save resp', resp); if(resp.success){ toastr.success(resp.message); // if row returned, refresh immediately
      if(resp.data){ // add or update row exists on server, reload list
        $('#noticeModal').modal('hide'); loadNotices();
      } else {
        $('#noticeModal').modal('hide'); setTimeout(loadNotices,200);
      }
    } else { toastr.error(resp.message||'Save failed'); } },'json').fail(function(jq){ console.error('save failed', jq.status, jq.responseText); }); });

  // Debug: log load results
  $(document).ajaxError(function(event, jqxhr, settings, thrownError){ console.error('AJAX error', settings.url, jqxhr.status, thrownError); });
