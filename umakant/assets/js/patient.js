...
$(function(){
      ...
      { 
        ...
        render: function(data, type, row){
          return `<div class="btn-group btn-group-sm action-buttons" role="group" aria-label="Actions" style="white-space:nowrap;">
                    <button class="btn btn-primary btn-action view-btn" data-id="${row.id}" title="View" onclick="viewPatient(${row.id})"><i class="fas fa-eye"></i></button>
                    <button class="btn btn-info btn-action edit-btn" data-id="${row.id}" title="Edit" onclick="editPatient(${row.id})"><i class="fas fa-edit"></i></button>
                    <button class="btn btn-danger del-btn" data-id="${row.id}" title="Delete" onclick="deletePatient(${row.id})"><i class="fas fa-trash"></i></button>
                  </div>`;
        } 
      }
    ],
  ...
  // Load users for the filter dropdown when the page loads
  loadAddedByUsers();

  // Handle click on Add New Patient button (now calls global openAddPatientModal)
  $(document).on('click', '#addPatientBtn', function() {
    console.log('#addPatientBtn clicked');
    openAddPatientModal(); // Call the global function
  });
  ...
  // Handle click on Print button in View Patient Modal
  $(document).on('click', '#printPatientDetailsBtn', function() {
...
function editPatient(id) {
  ...
  var table = window.patientTable; // Access the global DataTable instance
