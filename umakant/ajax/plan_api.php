...
if ($action === 'list'){
    $stmt = $pdo->query('SELECT p.id, p.name, p.description, p.price, p.upi, p.time_type, p.start_date, p.end_date, p.added_by, u.username as added_by_username FROM plans p LEFT JOIN users u ON p.added_by = u.id ORDER BY p.id DESC');
    $rows = $stmt->fetchAll();
